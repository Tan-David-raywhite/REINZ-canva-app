...
const ScrollableItems = ({ direction, fadeSize = '6u', gap = '2u', children, id, snap = 'proximity', snapAlign = 'start', snapStop = 'normal', padding, paddingX, paddingY, paddingTop, paddingBottom, paddingStart, paddingEnd, ref, scrollableRef, onScroll: onScrollProp, onScrollStateChange: onScrollStateChangeProp, itemSize = 'intrinsic', outOfViewItemBehavior = 'inert', role, ariaLabel, ariaLabelledBy, ariaDescribedBy, ariaSetSize, getItemWidth, itemCount, onFocus })=>{
    ...
    const paddingBefore = isHorizontal ? paddingStart || paddingX || padding : paddingTop || paddingY || padding;
    const paddingAfter = isHorizontal ? paddingEnd || paddingX || padding : paddingBottom || paddingY || padding;
    const paddingBeforeInPixel = useResponsiveUnitValueToPixel(paddingBefore || '0');
    const paddingAfterInPixel = useResponsiveUnitValueToPixel(paddingAfter || '0');
    const gapInPixel = useResponsiveUnitValueToPixel(gap);
    const { ref: scrollableItemRef, handle, onScroll } = (0, _behavior.useScrollableItems)({
        onScroll: onScrollProp,
        onScrollStateChange,
        gap: gapInPixel,
        paddingBefore: paddingBeforeInPixel,
        paddingAfter: paddingAfterInPixel,
        getItemWidth,
        itemCount
    });
    ...
    return (0, _jsxruntime.jsx)(ScrollableItemsContext, {
        value: contextValue,
...
function useResponsiveUnitValueToPixel(rawValue) {
    const responsiveValue = typeof rawValue === 'object' ? rawValue : {
        default: rawValue
    };
    const value = (0, _use_responsive_value.useResponsiveValue)(responsiveValue);
    return (0, _metrics.unitSizeToNumber)(value);
}
...
const ScrollableItem = _react.memo(
    function ScrollableItem({ children, snapAlign: itemSnapAlign, snapStop: itemSnapStop, size: itemSizeProp, role: roleProp, index }) {
        const { snapAlign: parentSnapAlign, snapStop: parentSnapStop, role: carouselRole, itemSize: parentItemSize, focusScrollContainer, numberOfItems } = _react.useContext(ScrollableItemsContext);
        const snapAlign = itemSnapAlign || parentSnapAlign;
        const snapStop = itemSnapStop || parentSnapStop;
        ...
        const itemClassName = (0, _classnames.default)(_scrollable_itemscss.default.item, {
            ...
            [_scrollable_itemscss.default.snapItemAlways]: snapStop === 'always'
