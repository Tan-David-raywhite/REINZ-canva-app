...
function SearchHeader({ children, isScrolled, sizingMode }) {
    const isPopover = sizingMode === 'fixed';
    const isSheet = sizingMode === 'fill';
    return (0, _jsxruntime.jsx)(_box.Box, {
        position: "relative",
        paddingX: "1u",
        paddingTop: isPopover ? '0.5u' : '1.5u',
        paddingBottom: isPopover ? '0.5u' : '0',
        background: isPopover ? 'surfaceFloating' : undefined,
        className: (0, _classnames.default)(_base_selectcss.default.searchHeader, {
            [_base_selectcss.default.scrolled]: isPopover && isScrolled,
            [_base_selectcss.default.sheet]: isSheet
        }),
        children: (0, _jsxruntime.jsx)(_box.Box, {
            borderRadius: "elementStandard",
            shadow: isSheet && isScrolled ? 'surfaceRaised' : undefined,
            className: _base_selectcss.default.searchInputWrapper,
            children: children
        })
    });
}
const BaseSelect = (0, _mobxreactlite.observer)(function BaseSelect(props) {
    const { Menu = _stateless_base_select.BaseSelectMenu, menu = (props)=> (0, _jsxruntime.jsx)(Menu, {
            ...props
        }), triggerContainerRef, menuContainerRef, searchable, isSelected = _base_select_util.defaultIsSelected, disabled, onChange, onOpen, onClose, closeOnSelection, renderSurface } = props;
    const [store] = _react.useState(()=>props.store ?? new _base_select_presenter.BaseSelectStore());
    const [presenter] = _react.useState(()=>props.presenter ?? new _base_select_presenter.BaseSelectPresenter());
    const open = props.open ?? store.open;
    const value = props.value ?? store.value;
    const { query } = store;
    const isSearchable = !!searchable;
    const { filterFn = _base_select_util.defaultFilterFn, onInputChange, inputPlaceholder, allowClear: searchableAllowClear = 'never', inputAriaLabel = _base_selectmessages.BaseSelectMessages.defaultSearchInputLabel() } = typeof searchable === 'object' ? searchable : {};
    ...
    const optionCount = _react.useMemo(()=>{
        return (0, _base_select_util.flattenOptions)(props.options).length;
    }, [
        props.options
    ]);
    const focusOnMenuContainer = (0, _sheet.useTypicalSheetAdaptation)() && !isSearchable && (renderSurface != null || optionCount > _surface.MAX_POPOVER_OPTION_COUNT);
    const focusOnMenu = focusOnMenuContainer || isSearchable;
    const defaultRenderSurface = (0, _surface.useDefaultSelectRenderSurface)({
        optionCount,
    ...
    const menuWithSearchInput = (props)=>menu({
            ...props,
            header: ({ isScrolled, sizingMode })=>(0, _jsxruntime.jsx)(SearchHeader, {
                    isScrolled: isScrolled,
                    sizingMode: sizingMode,
                    children: (0, _jsxruntime.jsx)(_base_input.BaseInput, {
                        type: "search",
                        value: store.query,
                        ...
                        borderless: sizingMode === 'fixed',
                        className: _base_selectcss.default.searchInput,
            ...
            children: props.options.length === 0 && query ? (0, _stateless_base_select.renderEmptyMenuContent)(query) : props.children
        });
    return (0, _jsxruntime.jsx)(_mobxreactlite.Observer, {
        children: ()=>(0, _jsxruntime.jsx)(_stateless_base_select.StatelessBaseSelect, {
                ...
                isMenuContainerFocusable: focusOnMenuContainer,
                triggerContainerRef: (0, _compose_refs.composeRefs)(open && focusOnMenu ? null : keyboardNavigation.ref, triggerContainerRef),
                menuContainerRef: (0, _compose_refs.composeRefs)(open && focusOnMenu ? keyboardNavigation.ref : null, menuContainerRef),
                menu: searchable ? menuWithSearchInput : menu,
                options: options,
                renderSurface: renderSurface ?? defaultRenderSurface,
