...
const _base_selectcss = _interop_require_wildcard(require("./base_select.css"));
...
const DEFAULT_POPOVER_WIDTH_UNITS = '40u';
const DEFAULT_POPOVER_WIDTH = parseInt(DEFAULT_POPOVER_WIDTH_UNITS, 10) * _metrics.baseUnit;
function StatelessBaseSelect(props) {
    const { Trigger = BaseSelectTrigger, Menu = BaseSelectMenu, Item, Popover = BaseSelectPopover, trigger = (props)=> (0, _jsxruntime.jsx)(Trigger, {
            ...props
        }), menu = (props)=> (0, _jsxruntime.jsx)(Menu, {
            ...props
        }), item, value, placeholder, open, disabled, error, stretch, activeIndex, options, size, focusOnMenu = false, popoverPlacement = 'bottom-start', popoverWidth: popoverWidthProp, blockOutsidePointerEvents, id, ariaLabelledBy, ariaDescribedBy, tooltipLabel, tooltipPlacement, tooltipDisabled, className, onRequestClose, onRequestOpen, onBlur, onFocus, onItemClick, onItemHover, areEqual = _base_select_util.defaultAreEqual, isSelected = _base_select_util.defaultIsSelected, getLabel = _base_select_util.defaultGetLabel } = props;
    ...
    const activeDescendantId = open ? (0, _base_select_util.deriveIdentifier)(menuId, activeIndex) : undefined;
    const ariaLabel = props.ariaLabel ?? (!ariaLabelledBy && typeof props.title === 'string' ? props.title : undefined);
    const [popoverWidth, setPopoverWidth] = _react.useState(popoverWidthProp ?? DEFAULT_POPOVER_WIDTH_UNITS);
    ...
    _react.useEffect(()=>{
        if (!stretch || popoverWidthProp != null) {
            setPopoverWidth(popoverWidthProp ?? DEFAULT_POPOVER_WIDTH_UNITS);
            return;
        }
        const triggerWidth = triggerContainerRef.current?.getBoundingClientRect().width ?? 0;
        const width = triggerWidth < DEFAULT_POPOVER_WIDTH ? DEFAULT_POPOVER_WIDTH_UNITS : 'trigger';
        setPopoverWidth(width);
    }, [
        popoverWidthProp,
        stretch
    ]);
    return (0, _jsxruntime.jsx)(Popover, {
...
function BaseSelectTrigger(props) {
    ...
    return (0, _jsxruntime.jsxs)(_base_button.BaseButton, {
        ...
        className: (0, _classnames.default)(_base_selectcss.default.triggerButton, (0, _base_selectcss.getStyle)(size), props.className),
        id: props.id,
        role: props.role,
