import { jsx as _jsx } from "react/jsx-runtime";
import { composeRefs } from '../../../../../base/react/compose_refs';
import { useIsomorphicLayoutEffect } from '../../../../../base/react/use_isomorphic_layout_effect';
import classNames from 'classnames';
import { action } from 'mobx';
import { Observer, observer } from 'mobx-react-lite';
import * as React from 'react';
import { useKeyboardNavigation } from '../../../a11y/keyboard_navigation/use_keyboard_navigation';
import { Box } from '../../../box/box';
import { BaseInput } from '../../base_input/base_input';
import { ClearDecorator } from '../../input_decorators/input_decorators';
import { useTypicalSheetAdaptation } from '../../../surface/sheet/sheet';
import styles from './base_select.css';
import { BaseSelectMessages } from './base_select.messages';
import { BaseSelectPresenter, BaseSelectStore } from './base_select_presenter';
import { defaultFilterFn, defaultIsSelected, deriveIdentifier, flattenOptions, normalizeOptionGroups } from './base_select_util';
import { BaseSelectMenu, renderEmptyMenuContent, StatelessBaseSelect } from './stateless_base_select';
import { MAX_POPOVER_OPTION_COUNT, useDefaultSelectRenderSurface } from './surface';
function SearchHeader({ children, isScrolled, sizingMode }) {
    const isPopover = sizingMode === 'fixed';
    const isSheet = sizingMode === 'fill';
    return _jsx(Box, {
        position: "relative",
        paddingX: "1u",
        paddingTop: isPopover ? '0.5u' : '1.5u',
        paddingBottom: isPopover ? '0.5u' : '0',
        background: isPopover ? 'surfaceFloating' : undefined,
        className: classNames(styles.searchHeader, {
            [styles.scrolled]: isPopover && isScrolled,
            [styles.sheet]: isSheet
        }),
        children: _jsx(Box, {
            borderRadius: "elementStandard",
            shadow: isSheet && isScrolled ? 'surfaceRaised' : undefined,
            className: styles.searchInputWrapper,
            children: children
        })
    });
}
export const BaseSelect = observer(function BaseSelect(props) {
    const { Menu = BaseSelectMenu, menu = (props)=> _jsx(Menu, {
            ...props
        }), triggerContainerRef, menuContainerRef, searchable, isSelected = defaultIsSelected, disabled, onChange, onOpen, onClose, closeOnSelection, renderSurface } = props;
    const [store] = React.useState(()=>props.store ?? new BaseSelectStore());
    const [presenter] = React.useState(()=>props.presenter ?? new BaseSelectPresenter());
    const open = props.open ?? store.open;
    const value = props.value ?? store.value;
    const { query } = store;
    const isSearchable = !!searchable;
    const { filterFn = defaultFilterFn, onInputChange, inputPlaceholder, allowClear: searchableAllowClear = 'never', inputAriaLabel = BaseSelectMessages.defaultSearchInputLabel() } = typeof searchable === 'object' ? searchable : {};
    const optionGroups = React.useMemo(()=>{
        return normalizeOptionGroups(props.options);
    }, [
        props.options
    ]);
    const options = React.useMemo(()=>{
        if (!isSearchable || !query) return props.options;
        return optionGroups.map((optionGroup)=>{
            const filteredOptions = filterFn(query, optionGroup.options, value, isSelected);
            return {
                label: optionGroup.label,
                options: filteredOptions
            };
        }).filter((optionGroup)=>optionGroup.options.length > 0);
    }, [
        query,
        isSearchable,
        filterFn,
        optionGroups,
        props.options,
        value,
        isSelected
    ]);
    const flatOptions = React.useMemo(()=>{
        return flattenOptions(options);
    }, [
        options
    ]);
    const optionCount = React.useMemo(()=>{
        return flattenOptions(props.options).length;
    }, [
        props.options
    ]);
    const focusOnMenuContainer = useTypicalSheetAdaptation() && !isSearchable && (renderSurface != null || optionCount > MAX_POPOVER_OPTION_COUNT);
    const focusOnMenu = focusOnMenuContainer || isSearchable;
    const defaultRenderSurface = useDefaultSelectRenderSurface({
        optionCount,
        searchable: isSearchable
    });
    const defaultOnSelection = React.useCallback((option, e)=>{
        if (!isSelected(option, value)) {
            presenter.setValue(store, option.value);
            onChange?.(option.value, option);
        }
    }, [
        isSelected,
        value,
        presenter,
        store,
        onChange
    ]);
    const onSelection = props.onSelection ?? defaultOnSelection;
    const openMenu = React.useCallback(()=>{
        if (!open) {
            presenter.toggle(store, true);
            onOpen?.();
        }
    }, [
        open,
        store,
        presenter,
        onOpen
    ]);
    const closeMenu = React.useCallback(()=>{
        if (open) {
            presenter.toggle(store, false);
            onClose?.();
        }
    }, [
        open,
        store,
        presenter,
        onClose
    ]);
    const initialActiveIndex = React.useRef(-1);
    const keyboardNavigation = useKeyboardNavigation(flatOptions.length, {
        handleInputs: true,
        onNext (e, index) {
            if (!open && !disabled) {
                initialActiveIndex.current = 0;
                openMenu();
            }
        },
        onPrev (e, index) {
            if (!open && !disabled) {
                initialActiveIndex.current = flatOptions.length - 1;
                openMenu();
            }
        },
        onSelect (e, index) {
            if (e.code === 'Space' && e.target instanceof HTMLInputElement && e.target.value.length > 0)
                return;
            e.preventDefault();
            if (!open) return openMenu();
            if (index === -1) return closeMenu();
            const activeOption = flatOptions[index];
            if (activeOption == null) return;
            if (activeOption.disabled) return;
            onSelection?.(activeOption, e);
            if (closeOnSelection === true)
                return closeMenu();
            if (closeOnSelection === false) return;
            if (e.code === 'Enter') closeMenu();
        },
        onExit (e, index) {
            if (!open) return;
            if (e.code === 'Escape' && e.target instanceof HTMLInputElement)
                return;
            closeMenu();
        }
    });
    const wasOpen = React.useRef(open);
    const { moveTo } = keyboardNavigation;
    useIsomorphicLayoutEffect(()=>{
        if (open && open !== wasOpen.current) {
            if (initialActiveIndex.current === -1) {
                const firstSelectedOptionIndex = flatOptions.findIndex((option)=>isSelected(option, value));
                if (firstSelectedOptionIndex > -1) initialActiveIndex.current = firstSelectedOptionIndex;
            }
            moveTo(initialActiveIndex.current);
        }
        wasOpen.current = open;
        initialActiveIndex.current = -1;
    });
    const onItemHover = React.useCallback((option)=>{
        moveTo(flatOptions.indexOf(option));
    }, [
        flatOptions,
        moveTo
    ]);
    const onItemClick = React.useCallback((option, e)=>{
        if (option.disabled) return;
        if (closeOnSelection !== false) closeMenu();
        onSelection(option, e);
    }, [
        onSelection,
        closeMenu,
        closeOnSelection
    ]);
    const onSearchInputChange = React.useCallback((value)=>{
        onInputChange?.(value);
        store.query = value.trimStart();
        moveTo(0);
    }, [
        onInputChange,
        moveTo,
        store
    ]);
    const menuWithSearchInput = (props)=>menu({
            ...props,
            header: ({ isScrolled, sizingMode })=>_jsx(SearchHeader, {
                    isScrolled: isScrolled,
                    sizingMode: sizingMode,
                    children: _jsx(BaseInput, {
                        type: "search",
                        value: store.query,
                        onChange: action(onSearchInputChange),
                        placeholder: inputPlaceholder,
                        ariaLabel: inputAriaLabel,
                        icon: {
                            type: 'search',
                            align: 'start'
                        },
                        borderless: sizingMode === 'fixed',
                        className: styles.searchInput,
                        ariaActiveDescendant: deriveIdentifier(props.id, props.activeIndex),
                        onKeyDown: (e)=>e.key === 'Tab' && e.preventDefault(),
                        end: searchableAllowClear !== 'never' && _jsx(ClearDecorator, {
                            when: searchableAllowClear === 'always' ? 'always' : 'not-empty'
                        })
                    })
                }),
            children: props.options.length === 0 && query ? renderEmptyMenuContent(query) : props.children
        });
    return _jsx(Observer, {
        children: ()=>_jsx(StatelessBaseSelect, {
                ...props,
                value: value,
                open: open,
                activeIndex: keyboardNavigation.index > -1 ? keyboardNavigation.index : undefined,
                onRequestClose: closeMenu,
                onRequestOpen: openMenu,
                onItemClick: onItemClick,
                onItemHover: onItemHover,
                focusOnMenu: focusOnMenu,
                isMenuContainerFocusable: focusOnMenuContainer,
                triggerContainerRef: composeRefs(open && focusOnMenu ? null : keyboardNavigation.ref, triggerContainerRef),
                menuContainerRef: composeRefs(open && focusOnMenu ? keyboardNavigation.ref : null, menuContainerRef),
                menu: searchable ? menuWithSearchInput : menu,
                options: options,
                renderSurface: renderSurface ?? defaultRenderSurface,
                tooltipLabel: props.tooltipLabel,
                tooltipPlacement: props.tooltipPlacement,
                tooltipDisabled: props.tooltipDisabled
            })
    });
});
