...
import { darkTheme } from '../tokens/canva_dark_theme';
import { lightTheme } from '../tokens/canva_light_theme';
import { colorSchemeClasses } from './color_scheme/color_scheme';
import preloadSelectors from './preload.css';
import styles from './theme.css';
export const stripGlobal = (className)=>className?.replace(/^\s*:global\(\.(.+?)\)\s*$/, '$1') ?? '';
let themeClasses;
let themedClass;
const getThemeClassName = (theme)=>Preconditions.checkExists(stylex.props(theme).className).split(' ');
const stylexThemeClasses = new Map([
    [
        'light',
        getThemeClassName(lightTheme)
    ],
    [
        'dark',
        getThemeClassName(darkTheme)
    ]
]);
...
export function getThemeClasses(theme) {
    ...
    return [
        ...
        ...Preconditions.checkExists(stylexThemeClasses.get(theme)),
        ...Preconditions.checkExists(colorSchemeClasses.get(theme))
    ];
}
