...
const InternalVideoCard = (props)=>{
    ...
    const ref = isVideoProps(props) ? props.ref : undefined;
    const video = React.useRef(null);
    const setVideoRef = React.useCallback((node)=>{
        if (node != null) video.current = node;
    }, []);
    const [status, setStatus] = React.useState('uncontrolled');
    ...
    React.useEffect(()=>{
        const dispose = ()=>video.current?.pause();
        return dispose;
    }, []);
    const whenToPlay = status === 'uncontrolled' ? 'on-hover' : 'now';
    const autoplay = status !== 'paused';
    const commonProps = {
