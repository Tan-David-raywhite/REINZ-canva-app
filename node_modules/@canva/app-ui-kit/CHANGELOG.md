...
- Added a `ref` property to the `VideoCard` component to enable programmatic control of preview playback.

### Changed

- When `MenuDivider` renders with a text child element, it now has less vertical padding. Minor visual changes apply, including where it is used for group headings in the `Select` component.

### Fixed

- Fixed a rendering issue in the `TabList` component for overflow scroll arrows.
- The `Link` component's pressed-state now correctly displays on hover-capable devices.

## 5.12.0 - 2026-07-02

### Changed

- On mobile, the `Select` and `CustomizableSelect` components now show short lists (6 or fewer options) as a flyout instead of a sheet. Searchable and longer lists still use a sheet.
- The `Select` component now uses an improved focus-management pattern in sheet mode so it works better with screen readers.
- The `Button` loading shimmer animation has been refined. Minor visual changes may appear.
- The `PillsInput` loading state now uses a shimmer effect instead of a spinner animation.

### Fixed

- The `LoadingIndicator` is centered again, fixing a regression in 5.11.0 where it was left-aligned.
- The `Select` component now correctly scrolls the sheet menu to the active option when it is opened.
- Fixed an issue in the `FlyoutMenu` component where closing the menu with the Tab key caused multiple tab jumps.
- The `SegmentedControl` component now keeps the divider visible when hovering over a disabled item, instead of fading it out.
- The `PillsInput` component now sets `aria-invalid` on the input when the `error` prop is set, improving screen-reader accessibility.
- Fixed an issue preventing the background gradient of the `ColorSelector` showing the full value range. Minor visual changes apply.
- Fixed scroll padding in mobile sheet surfaces (such as `Select` and `FlyoutMenu`) so scrolling works as expected with sticky headers and footers.

## 5.11.0 - 2026-06-18

### Deprecated

- Deprecated the `className` prop on `Box` and the `CustomizableSelect` trigger/item. These props will be removed in a future major release. Prefer design tokens and layout props, or wrap it in a standard HTML element if you need custom CSS classes.

### Added

- Added `stylexjs/stylex` as a dependency, some internal components now transition to use stylex. No impact to external consumers.

### Changed

- Small visual change to the `MusicIcon`.
- Migrated `react-measure` dependency to an internal measure module, no external impact expected.

### Fixed

- Fixed an issue with the `FlyoutMenu` component where extra, unknown props were being passed to DOM nodes, potentially causing unexpected behavior. This change prevents unknown props from being passed to DOM nodes.

## 5.10.0 - 2026-06-03

### Changed

- App UI Kit components now apply minor visual changes for Canva's modernised UI refresh. Apps on `v5.8.0` and above should have already had these changes rolled out slowly over the recent weeks. Changes include updated border radius, tactility / micro-interaction, refined elevation, and improved dark mode.
- Behavioral improvement to the `Carousel` component, the ScrollButton no longer impacts the layout surrounding the Carousel. No visual change but the clickable area of the button is slightly shifted.

### Added

- Added `@canva/app-ui-kit/icons` entrypoint for importing App UI Kit icons separately from the main package.

### Fixed

- Fixed the searchable `Select` components focus ring to appear on the trigger instead of the search input, matching the behavior of non-searchable selects.
